 ...
 * 添加宝贝布局界面
 * 
 * @author binbird 姜斌 （binbird@qq.com）
 * @version 2.0
 * @created 2015-1-31
 */
public class AddbabyActivity extends BaseSinglePhotoActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//加载省市联动
		regional.loadSpinner(AddbabyActivity.this,province_spinner,city_spinner,county_spinner);
...
//			Regional.setSpinnerItemSelectedByValue(city_spinner, a3);
		...
		add_baby_back_btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(tag.equals("0")){
					...
					appContext.Logout();
					UIHelper.showLogin(AddbabyActivity.this);
					finish();
				}else{
					finish();
				}
	...
	private void initData() {
		...
		titleRightButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				UIHelper.showhomepage(AddbabyActivity.this);
				finish();
			}
		});
		...
		add_baby_save_btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (add_baby_name_edit.getText().toString().equals("")) {
					UIHelper.ToastMessage(AddbabyActivity.this, "baby名字不能为空！");
				} else if (add_baby_name1_edit.getText().toString().equals("")) {
					UIHelper.ToastMessage(AddbabyActivity.this, "baby昵称不能为空！");
				} else {
					showDialog(SIGN);
					...
					if(bitmaptemp != null){
						FileUtils fileutils = new FileUtils(AddbabyActivity.this);
						File file = null;
						try {
		...
		new Thread() {
			public void run() {
				...
				try {
					...
					paramMap.put("Province", baby.getBnativeplace1());
					paramMap.put("City", baby.getBnativeplace2());
					paramMap.put("Country", baby.getBnativeplace3());
		...
		final Handler handler = new Handler() {
			public void handleMessage(Message msg) {
				...
				if (msg.what == 1) {
					...
					Toast.makeText(AddbabyActivity.this, "获取数据失败0", Toast.LENGTH_SHORT).show();
				} else if (msg.what == 0) {
					...
					Regional.setProvinceCityItemSelectedByValue(province_spinner, a2,a3,a1);
					...
					Toast.makeText(AddbabyActivity.this, "获取数据失败-1", Toast.LENGTH_SHORT).show();
				}
			}
		};
		new Thread() {
			public void run() {
				...
				try {
					...
					AppContext ac = (AppContext) AddbabyActivity.this.getApplication();
