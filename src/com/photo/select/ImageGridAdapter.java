...
import com.photo.select.BitmapCache.ImageCallback;
...
public class ImageGridAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		cache.displayBmp(holder.iv, item.thumbnailPath, item.imagePath,
				callback);
		if (item.isSelected) {
			holder.selected.setImageResource(R.drawable.photo_icon_data_select);  
		...
		holder.iv.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if ((Bimp.drr.size() + selectTotal) < 9) {
					...
					if (item.isSelected) {
						holder.selected
								.setImageResource(R.drawable.photo_icon_data_select);
						holder.text.setBackgroundResource(R.drawable.photo_bgd_relatly_line);
						selectTotal++;
